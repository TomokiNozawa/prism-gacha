...
const SW_VERSION = '1.4.3k';  // 画像永続cache 分離 (Ver bump 時の全DL し直し問題を解消、 野沢さん指摘 2026-05-03)
...
async function staleWhileRevalidate(req, cacheName) {
  ...
  // ignoreSearch:true で cache buster (?v=1.4.3k) の差異も許容 → 「DL したのに別 buster で読まれて遅い」 事故防止
  try {
  ...
  // 同 path で cache hit すれば再DL不要、 query (?v=1.4.3k) は無視
