...
async function loadMasters() {
  const [c, k, l, p, eo] = await Promise.all([
    fetch('./cards.json?v=1.5.1q').then(r => r.json()),
    fetch('./combos.json?v=1.5.1q').then(r => r.json()),
    fetch('./lane_effects.json?v=1.5.1q').then(r => r.json()),
    fetch('./data/pool.json?v=1.5.1q').then(r => r.json()).catch(() => []),
    fetch('./effects_override.json?v=1.5.1q').then(r => r.json()).catch(() => ({})),
  ]);
