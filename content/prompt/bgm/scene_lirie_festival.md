# シーン特化 BGM プロンプト — Prism Shrine (巫女連邦の灯篭祭)

「その他」 カテゴリ (シーン特化テーマ)。 巫女連邦リーリエの灯篭祭 (s1c6 1-2「セラフィエル降臨——巫女連邦の灯篭祭」 シーン専用)。 Suno AI 向け。 既に Suno 生成済 + 取込済 (prism-shrine.mp3 BPM 128 神楽の喜び joyful kagura)。

> **🎯 引き継ぎ規約**
> - 出力ファイル: `media/audio/bgm/prism-shrine.mp3` (✅ 生成済 + 取込済)
> - **カテゴリ**: `other` (BGM_LIST `category: 'other'`、 戦闘テーマ rift / 銀霜国月夜祭 lullaby と並列のシーン特化、 派閥 BGM ではない)
> - **基調**: アップテンポ祝祭神楽 joyful kagura (BPM 128、 D major Lydian/Mixolydian、 民の祝祭の活気)
> - 章テーマ prism-promise (BPM 118 七座満つる神聖儀式、 angelic 格調) とは別軸、 章本文 1-2 灯篭祭シーンの音楽そのもの
> - **派閥 BGM は別途**: 巫女連邦リーリエ派閥 BGM (神楽中心の常態) は `faction_lirie.md` で 別ファイル管理、 別曲 (野沢さん指示 2026-05-07「灯篭祭の BGM っぽい、 もっと巫女感強いのを派閥テーマに」)

---

## 楽曲コンセプト

巫女連邦の **灯篭祭** シーン特化 BGM。 s1c6 1-2「セラフィエル降臨——巫女連邦の灯篭祭」 シーンそのままを音楽化。 朝の境内で巫女達が神楽を舞い、 七色の紙灯篭が日中の光に揺れ、 弟子達の笑い声と先達巫女の温かい指導が響く、 千年継承された生きた伝統の喜び。

「その他」 カテゴリ配置の理由: 巫女連邦 **全体** を代表する派閥 BGM ではなく、 **特定シーン (灯篭祭の祝祭夜)** 専用なので、 戦闘 BGM rift / 銀霜国月夜祭 lullaby と同様の シーン特化テーマとして「その他」 に置く。 巫女連邦の派閥 BGM は神楽中心の常態 (Prism Kagura) で別途 `faction_lirie.md` で制作。
